...
final extension of the black surface is not so high. The wideer the two blocks, the wider the
...
Let's try another configuration: we can still work with two blocks, but try to make them a
...
When the CA evolves, this configuration will stabilize into a final condition with a pretty well
extended final black surface. It also seems like, if the blocks get wider in one direction
(higher width, or higher height), the final result is also better. However the problem is the same,
