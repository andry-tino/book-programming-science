...
\begin{learnp}{A first glance at HTML}
...
\begin{learnp}[sec:learn:jsmodules]{Javascript modules}
Javascript does not have a concept of \textit{module} embedded in itself,
...
Everytime we write some code, we will wrap it inside a module like shown before
because it is a good practice and a standard procedure to make sure other
Javascript codes loaded in the page, in future, do not
affect our code.
\end{learnp}
...
\begin{learnp}{Again on Javascript modules}
...
Moving on, we now want to create the code that renders the grid.
...
\begin{programcode}[Snippet]{style.css}
...
\begin{learnp}{Signature of a function}
...
\begin{figure*}[h]
  ...
  some of the most famous initial configurations (or \textit{lifes} as Conway calls them) in CGL.
  The top row, shows five static configurations: when the automaton starts from those,
  it will not have an evolution.
  The middle row shows periodic (recurrent) configurations with period $T=2$: it means that every
  2 cycles, the CA will get back to these configurations to start over again indefinitely.
  The bottom row shows divergent configurations which are very interesting: when the
